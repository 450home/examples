"""End-to-end test for the ``node-playwright-firefox`` example.

Mirrors the manual steps from ``node-playwright-firefox/README.md``:

1. ``unikraft build . --output <prefix>/node-playwright-firefox:<tag>``
...
def test_node_playwright_firefox_screenshot(build_image, run_instance, http):
    image = build_image(
