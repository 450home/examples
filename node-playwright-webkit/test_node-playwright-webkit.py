"""End-to-end test for the ``node-playwright-webkit`` example.

Mirrors the manual steps from ``node-playwright-webkit/README.md``:

1. ``unikraft build . --output <prefix>/node-playwright-webkit:<tag>``
...
def test_node_playwright_webkit_screenshot(build_image, run_instance, http):
    image = build_image(
